...
In order to join the data sets, we must first pivot the data. Each data set contains one column with the country, and every other column is the year. Performing a join with the original data sets would make further analysis difficult, since there would be multiple of the same years, and it would be difficult to differentiate between which year column tells us what.
...
Now that we have pivoted the data, we can join the new data sets to perform our analysis. We will be using an inner join so we can keep only the data that is present in both data sets.
