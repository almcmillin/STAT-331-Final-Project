...
```{r}
coefs <- as.numeric(coef(life_expectancy_lm))  
residual_sd <- summary(life_expectancy_lm)$sigma
...
r2_values <- numeric(num_simulations)

set.seed(4403)  
for (i in 1:num_simulations) {
  simulated_life_expectancy <- rand_error(
    coefs[1] + coefs[2] * full_data_avg$avg_seafood_consumption,
    sd = residual_sd)
  
  sim_model <- lm(full_data_avg$avg_life_expectancy ~ simulated_life_expectancy)  
  r2_values[i] <- summary(sim_model)$r.squared
}

r2_df <- data.frame(R2 = r2_values)

ggplot(r2_df, aes(x = R2)) +
  geom_histogram(binwidth = 0.02, fill = "turquoise", color = "black") +
  labs(title = "Histogram of R^2 Values from Simulation",
       x = "R^2 Value", y = "Frequency") +
  theme_minimal()
```

In the graph above we compare our fitted model with a models with random variance in which it output $R^2$ values that represents the amount of variance explained from our fitted model. As we can see, a large proportion of the R2  values (600+) were 0.00 meaning that nearly none of the variance was explained by the model and it more simmlar to the random variance model. The graph is heavily skewed to the right meaning that there is little to no instances where that graph explains the variance for the data. Seeing that we have extremely low R2 values, we have strong evidence that seafood consumption is not correlated with life expectancy. 
